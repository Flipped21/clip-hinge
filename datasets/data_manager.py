...
class DummyDataset(Dataset):
    def __init__(self, images, labels, transforms=None):
        ...
        # 根据标签数量动态计算tasks
        # 修复：应该根据类别ID直接计算任务ID，而不是根据唯一类别数
        # 每个任务有 10 个类别（对于 CIFAR-100），20 个类别（对于 ImageNet-R）
        class_num = 10 if len(set(labels)) <= 100 else 20  # 根据数据集类型判断
        self.tasks = labels // class_num
